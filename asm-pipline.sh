...
echo "[*] Preparing output files & path..." | tee ${log_file}
...
if [ -z ${!current_progress} ]
then
	cmd="${tagmeth_cmd} ${clean_bam_filename} ${ref_fa} -o ${tagmeth_file}"
	echo "    CMD - ${cmd}" | tee -a ${log_file}
	${cmd} >> ${log_file} 2>&1
...
then
	...
	awk -v path=${chr_tagmeth_path} ' NR!=1 {print >> path"/"$4".tagmeth.csv"}' ${clean_tagmeth_file} 
	save_progress ${current_progress}
fi
